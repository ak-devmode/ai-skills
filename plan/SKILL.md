...
> `<plan-stem>-PROGRESS.md`. See §2.3 (resolution) + §4.2 (subsection schema) +
> §2.3.1 (migration of legacy files). This fixes a class of bugs where /plan invocations
> wrote per-task entries to a different file than the one the next invocation read.
...
- Also check inside scope subdirectories for plan stubs generated by /scope.
  Scope folders are named `<N>-<slug>/` (e.g. `32-pmg-testsuite/`); older
  archived scopes may use the legacy `scope-<slug>/` form.
...
2.3.1 **Migration of pre-3.3.0 child-plan progress files.** On Phase 0, if a child plan has both:
- a `<plan-stem>-PROGRESS.md` file in the scope folder, AND
- the scope's `progress.md` lacks a subsection for that plan

Then migrate before proceeding:

1. Read both files.
2. Insert a `## Plan {N}.{P}: {Title}` subsection into the scope `progress.md` (after the scope-level Resume Context block but before the Decisions Log) containing the full content of the legacy plan progress file (Resume Context, all Session blocks, all Task entries).
3. Add a one-line note at the top of the migrated subsection: `**Migrated from `{plan-stem}-PROGRESS.md` on {ISO timestamp} — file deleted.**`
4. Delete the legacy `<plan-stem>-PROGRESS.md` file.
5. Log the migration in the new subsection's first Session block under "Issues".

After migration, all subsequent progress writes for this plan go to the scope `progress.md` subsection. Idempotent: re-running Phase 0 on an already-migrated plan finds the subsection and skips migration.
