...
> (§11.8.4/§11.8.5) is now gated on the `/ready-to-clear` skill: a fresh subagent
...
> unplanned side-path work to be logged at the moment it happens; §11.6 closeout
...
`/markdown-style` §8 owns the shape — header fields, task blocks, checkpoint format,
workspace-relative paths, child-plan sub-numbering. Read it there rather than from a
second copy here.

The three fields this skill *acts* on:

- **`Status`** must read `Ready to execute`. `Draft` fails pre-flight (§5.3).
- **`Type`** per task: `AI` (autonomous) · `HUMAN` (stop and wait) · `AI+HUMAN_REVIEW`
  (execute, then stop for review before marking complete).
- **`Gate`** on each phase CHECKPOINT: A human · B concurrency · C review/merge ·
  D deploy/verify · E irreversible · F compliance. Set by `/scope`, read here to decide
  whether to suggest `/clear` (§11.8.4). A plan predating gate tagging is treated as B/C.
...
**Gate.** `/plan` §11 closeout-prep must not mark a plan Complete while its
...
`/markdown-style` §10 owns the schema for both modes. What is specific to this skill is
**where** writes go, resolved in §2.3:

- **Child plan** → a `## Plan {N}.{P}: {Title}` section inside the scope's `progress.md`,
  with its own `### Resume Context (Plan {N}.{P})` at the top and `### Session:` blocks
  beneath. Cross-plan content — the Plans table, Decisions Log, Human Steps, the
  scope-level Progress Log — stays at top level alongside the plan sections.
- **Standalone plan** → its own `<plan-stem>-PROGRESS.md`, single Resume Context at top.

**Two Resume Contexts, both live.** When a child plan advances, update its subsection's
block *and* the scope-level one, whose "Next action" points at the active plan's next
task. Updating the scope-level block is not overwriting plan content — they are separate
blocks at separate scopes.

Everything except a Resume Context block is append-only.
...
On the first invocation against a plan with no progress recorded: discover the plan (§2),
confirm it parses and count phases/tasks/checkpoints, create or extend the progress
destination per §2.3, print the status summary (§6.7), then run Phase 0 (§5) before
touching Task 1.1.
...
Create `TO-DO.md` with a one-line header if absent, then append a
`## {Plan Title} (Plan {N})` section with one `- [ ]` per deferred item and a
`Source:` line pointing at the archived progress file. If nothing was deferred, say so
explicitly in that section rather than omitting it — an absent section is
indistinguishable from an unrun step.
...
One completion summary: plan number and title, where it now lives (archive path for a
standalone plan; "stays in {scope-folder}, archives with the scope" for a child), how
many TODOs went to `TO-DO.md`, the index status change, and — for a child — that the
parent scope's progress.md was updated.
