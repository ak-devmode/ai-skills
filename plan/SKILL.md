...
Execute tasks from a plan document, track progress in the resolved progress destination,
and stop at human checkpoints. A plan is **one phase — a unit of deliverable work bounded
by a gate** (human / concurrency / review / deploy / irreversible / compliance — see
/scope "Phase Boundaries"), not a unit sized to a context window. A plan may span several
sessions: automatic compaction carries the working set, and intra-phase checkpoints let a
fresh session resume mid-plan. Plans may be standalone or generated by /scope as phase stubs.

> **v3.6.0 — scripts own the deterministic steps.** Plans-dir resolution, plan
> discovery and PLANS-INDEX writes now call `ai-skills/scripts/*` instead of
> describing themselves in prose (see `scripts/README.md`). §1.1 gains the grep-don't-read
> rule for large canonical docs. Removed: the Ralph Loop section (harness owns looping),
> §5.12's working-memory spec, §7.3's no-op caching note, §7.6's approval state machine,
> and §2.3.1's completed migration.
>
> **v3.5.1 — canonical-sources preflight (§1.1).** Consult project docs
> (FLEET.md / ARCHITECTURE.md / PLANS-INDEX.md / repo CLAUDE.md) before probing
> infra/architecture — probing to discover what a doc already records is a defect.
>
> **v3.5.0 — clear-readiness gate + side-path logging.** Every `/clear` suggestion
> (§12.8.4/§12.8.5) is now gated on the `/ready-to-clear` skill: a fresh subagent
> validates disk truth (progress, ledger, index) against git truth and must pass a
> cold-resume reconstruction before clearing is suggested. New §8.10 requires
> unplanned side-path work to be logged at the moment it happens; §12.6 closeout
> deferrals now write durable TO-DO.md entries. This targets the dropped-work class
> where end-of-context sessions suggest clearing with unfinished bookkeeping.
>
> **v3.3.0 — single source of truth for progress.** Child plans no longer have their
> own `<plan-stem>-PROGRESS.md` file. Their progress lives as a `## Plan {N}.{P}: ...`
> subsection inside the scope's `progress.md`. Standalone plans still use
> `<plan-stem>-PROGRESS.md`. See §2.3 (resolution) + §4.2 (subsection schema). This fixes
> a class of bugs where /plan invocations wrote per-task entries to a different file than
> the one the next invocation read. (The one-time migration routine was removed
> 2026-08-09 — no legacy files remain.)
...
## 1.1 Canonical sources — consult BEFORE probing (anti-archaeology)

**Probing to discover something a project doc already records is a defect.** Before
shelling out to inspect infra, hosts, DB/DSN, docker, ports, deploy paths, SSM layout,
or architecture — or making an assumption about any of them — read the canonical source.
Do not reverse-engineer via SSM/AWS/`docker`/grep what a doc states directly.

| Need | Canonical source (read first) |
|---|---|
...
- By number: look for `{N}.*-PLAN.md` or `{N}-*-PLAN.md` in the plans directory and
  inside scope subdirectories (named `<N>-<slug>/` per the current convention, or
  legacy `scope-<slug>/` in older archives). Example: "plan 39.2" → find
  `39.2-*-PLAN.md`.
...
Plans directory comes from the script, not from a hardcoded list:

```bash
PLANS_DIR="$(~/Projects/ai-skills/scripts/resolve-plans-dir.sh)" || exit
```

Also check inside scope subdirectories for plan stubs generated by /scope. Scope
folders are named `<N>-<slug>/` (e.g. `32-pmg-testsuite/`); older archived scopes may
use the legacy `scope-<slug>/` form.
...
2.3 **Progress file resolution — scope-aware (v3.3.0+).**

The destination for progress writes depends on whether the plan is a **child of a scope** or **standalone**. This rule eliminates the dual-file ambiguity that caused per-task progress entries to be lost when /plan invocations targeted the scope folder.

- **Child plan** (lives inside a `<N>-<slug>/` scope folder, plan has a `**Parent scope:**` field, OR a `scope.md` exists in the same directory): write progress to the **scope's `progress.md`**. The child plan owns a dedicated subsection inside the scope progress file — see §4 for the subsection schema. **Do NOT create a separate `<plan-stem>-PROGRESS.md` file.** If one already exists from a prior session, migrate its content into a subsection inside `progress.md` and delete the stub (see §2.3.1).
- **Standalone plan** (lives in its own folder with no `scope.md` sibling): write progress to a dedicated `<plan-stem>-PROGRESS.md` file co-located with the plan. This is the legacy single-file mode.

Naming for standalone plans:
- `39.2-foo-bar-PLAN.md` → `39.2-foo-bar-PROGRESS.md`
- `foo-bar-PLAN.md` → `foo-bar-PROGRESS.md`
- `PLAN.md` → `PROGRESS.md`

2.3.1 **The pre-3.3.0 migration is done — removed 2026-08-09.** This section held a
five-step routine for folding a legacy `<plan-stem>-PROGRESS.md` into the scope's
`progress.md`. Verified on 2026-08-09: **zero** non-archived `*-PROGRESS.md` files
remain across `wellmed/kalpa-docs/plans`, `pmg/pmg-docs/plans`, and
`ai-skills/plans`. The branch could never execute again, and it was read on every
Phase 0.

If a legacy file ever reappears (an old branch, a restored backup): fold its content
into a `## Plan {N}.{P}` subsection of the scope `progress.md` per §4.2, note where
it came from, delete the stub. Nothing more elaborate is needed for a one-off.
...
**If the plan lives inside a scope folder (named `<N>-<slug>/`, or legacy
`scope-<slug>/`):** No action needed — `/scope` already created the folder
hierarchy.
...
The CHECKPOINT's **Gate** field (set by /scope) classifies the phase boundary. It
...
See `/markdown-style` §10 (Progress Files) for full conventions: Resume Context block schema, append-only rule, Decisions Log dual-entry, workspace-relative paths.

The format depends on whether the plan is a **child of a scope** (writes to scope `progress.md` as a subsection) or **standalone** (writes to its own `<plan-stem>-PROGRESS.md`). See §2.3 for resolution rules.

### 4.1 Standalone plan — single-file schema
...
### 4.2 Child plan — per-plan subsection inside scope `progress.md`

For a scope with multiple child plans, `progress.md` has this overall structure:
...
**Last action:** {scope-level summary of most recent activity across all plans}
**Next action:** {which plan to advance + which task}
**Open blockers:** {or "None"}
**Key files changed:** {workspace-relative paths, most recent first}
...
### Resume Context (Plan {N}.1)
**Plan:** {path to plan file}
**Last action:** {plan-scoped: what just happened in this plan}
**Next action:** {next task in this plan}
**Open blockers:** {or "None"}
**Key files changed:** {workspace-relative paths for this plan}
...
### Resume Context (Plan {N}.2)
...
```

Rules for child-plan subsections:

- **Each child plan owns a top-level `## Plan {N}.{P}: {Title}` section.** Heading depth one level deeper than the scope-level Resume Context.
- **Each plan subsection has its own Resume Context at the top** (heading `### Resume Context (Plan {N}.{P})`). This is the only overwritable content inside the subsection.
- **Session blocks live at `### Session: ...` depth inside the subsection.** Task entries are `####`.
- **Cross-plan content stays at scope level:** Plans table, Decisions Log, Human Steps, Progress Log table — these remain under top-level `##` headings in `progress.md` alongside the plan subsections.
- **Scope-level Resume Context** (top of file) summarizes overall scope state — which plan is active, last cross-plan event. Update it whenever a plan transitions (start, checkpoint, complete) OR whenever the active plan's Resume Context updates. The scope Resume Context's "Next action" points at the active plan's next task.

### 4.3 Append-only rule (both modes)

The Resume Context block (whether top-of-file for standalone, or top-of-subsection for child plans) is the **only** section that gets overwritten. Everything else is append-only.

Updating the scope-level Resume Context when a child plan changes state does NOT count as overwriting plan-subsection content — those are two separate Resume Context blocks at different scopes.
...
6.1 On each invocation, read the plan file and the **resolved progress file** (per §2.3 — scope `progress.md` for child plans, `<plan-stem>-PROGRESS.md` for standalone). Find the first task that does NOT have a corresponding entry in the progress file (or has a ❌ FAILED entry that should be retried).

For child plans, "the progress file" specifically means the `## Plan {N}.{P}: ...` subsection of the scope `progress.md`. Do not write progress entries to the legacy `<plan-stem>-PROGRESS.md` file even if it exists — migrate per §2.3.1 instead.
...
1. Update the **resolved progress file** with the result — for child plans this means appending a Task entry inside the `## Plan {N}.{P}: ...` subsection AND refreshing both the plan-subsection Resume Context and the scope-level Resume Context. For standalone plans, just append + refresh the single top-level Resume Context.
...
Then proceed to the next task — keep going until you hit a HUMAN task, a CHECKPOINT, or the end of the current phase. Do not advance past a task without completing the progress write and the ledger append.
...
### 7.6 Don't re-ask the same question

Once the user has approved a novel pattern, apply the same decision to same-shape
methods for the rest of the session without halting again, and note in each §3 entry
which approval it followed. Approvals do not survive the session: a cross-session
pattern decision belongs in code or in CROSS-REPO.md, not in skill state.
...
8.2 **Always append to the resolved progress file.** Never delete or overwrite previous entries (Resume Context blocks excepted, per §4.3). The log is an audit trail. For child plans, "the progress file" means the `## Plan {N}.{P}: ...` subsection of the scope `progress.md` — see §2.3 + §4.2. Never write child-plan task entries to a `<plan-stem>-PROGRESS.md` file.
...
8.10 **Unplanned-work logging (side-path rule).** Any change made outside the active task's defined Output — a drive-by bugfix, a config repair, a doc correction, a test un-breaking — gets logged **at the moment it happens**, not at session end:
- Append a progress entry under the current Session block: `#### Unplanned: {short description}` with **Files modified** and one line of why.
- Append the same files to `closeout-prep.md §2` (mark `(unplanned)`), and to §11 Risk Flags if the fix is partial or untested.
- If the side path grows beyond ~15 minutes or touches another repo, STOP — surface it to the user as a candidate TO-DO.md entry or new scope instead of continuing silently.
The `/ready-to-clear` validator (check C) diffs git-changed files against logged files; anything unlogged fails the clear gate. This rule is what makes that check pass honestly rather than retroactively.
...
8.7 **Update parent scope on completion.** When all tasks in the plan are complete and the plan has a parent scope, update the parent scope's `progress.md` (which already contains the child-plan subsection per §4.2):
- Add a row to the scope-level Progress Log: `| {date} | /plan | Done | {plan filename} complete — {summary} |`
...
## 9. Autonomous / Looped Execution

`/plan` is already resumable from its progress file, which is all a loop needs: each
invocation reads progress, executes the next task, logs, and stops at a HUMAN task or
CHECKPOINT. To run it unattended, use the harness — `/loop` for a self-paced or
interval cadence, or a background task that re-invokes on completion. There is no
special prompt template to memorise.

> Removed 2026-08-09: a ralph-loop plugin integration section with a hand-written
> prompt template and `<promise>PHASE_COMPLETE</promise>` sentinels. The harness owns
> looping now, and the sentinels were never read by anything in this repo.

## 10. First Run Setup

If the resolved progress destination (per §2.3) does not yet contain progress for this plan when this skill is first invoked:
...
10.3 Create or extend the progress destination per §2.3:
- **Standalone plan:** Create `<plan-stem>-PROGRESS.md` with the single-file header from §4.1 + session start timestamp.
- **Child plan, scope `progress.md` doesn't exist yet:** Create scope `progress.md` with the scope-level header + add the first `## Plan {N}.{P}: ...` subsection per §4.2.
- **Child plan, scope `progress.md` exists but lacks this plan's subsection:** Append a new `## Plan {N}.{P}: ...` subsection after any existing plan subsections, before the Decisions Log + Progress Log tables. Update the scope-level Resume Context to point at this newly-active plan.
- **Child plan, subsection already exists:** Resume — refresh the subsection Resume Context with current state, append a new `### Session: {timestamp}` block.

10.4 Print the full status summary including total scope.

10.5 Run Phase 0 pre-flight (Section 5) before beginning task execution. This includes the §2.3.1 migration check for any legacy `<plan-stem>-PROGRESS.md` files.
...
`progress.md` (per §4.2) — the scope manages archival of the entire folder when
ALL child plans are complete (via /scope Step 7). Individual child plan
completion is tracked by status updates only: mark Done in PLANS-INDEX, update
the parent scope's progress.md Plans table + scope-level Resume Context, mark
the child plan's subsection Resume Context as "Plan complete — N/N tasks done",
and extract TODOs. Skip §12.3.1 through §12.3.2 entirely for child plans.
...
For child plans (per §2.3), all progress already lives in the scope `progress.md` as a subsection. On plan completion:
- Add a row to the scope-level Progress Log: `| {date} | /plan | Done | {plan filename} complete — {summary}. TODOs extracted to TO-DO.md |`
- Update the scope-level Plans table status to `Done`
...
If the user says yes to either, invoke that skill. If no, the deferral must be
**durable, not a passing note** — closeout deferrals are a documented dropped-work
class. Record it in BOTH places:
- Append to `TO-DO.md` under this plan's section: `- [ ] Run /closeout for plan {N}
  — deferred at completion on {date}`
- Add to the Resume Context "Open blockers" line: `closeout deferred (see TO-DO.md)`

If neither /closeout nor /closeout-extended is installed yet, fall back to
`/review` and note the gap.
...
- **A (human) / D (deploy-verify) / E (irreversible)** — there's a natural pause here
  (you're waiting on a person, a deploy, or pausing before a one-way step), so this is
  ...
  > Recommend running `/clear` then: `/plan {next-plan-number}`

- **B (concurrency) / C (review-sized) / F (already-cleared compliance)** or no Gate
  field — do **not** prompt to clear. Roll straight into the next plan. Compaction
  handles context; a forced reset here just costs you re-loading shared context.
