...
into a `## Plan {N}.{P}` subsection of the scope `progress.md` per §4, note where
...
- **Never execute tasks on `main` or `master`** UNLESS the repo's recent commit history shows main is the active development branch (e.g., solo-dev repos where all features land directly on main). When uncertain, halt and ask. Working on the user's current branch is preferred over creating new ones.
...
8.7 **Update parent scope on completion.** When all tasks in the plan are complete and the plan has a parent scope, update the parent scope's `progress.md` (which already contains the child-plan subsection per §4):
...
progress entry with where it actually lives, drop it from the index row's remaining
list, and do NOT extract it. One grep per item. Grep the **trunk**, not the working tree
or the ledger — work landed by another scope is invisible in both:
`git -C <repo> grep -l '<symbol>' origin/<trunk>`. Content-grep; don't trust a PR's
merge status.
...
`progress.md` (per §4) — the scope manages archival of the entire folder when
...
> **Why a script.** A prose "append a row if none exists" step, with no header
