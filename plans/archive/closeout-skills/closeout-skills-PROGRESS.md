...
**Last action:** **Phase 5 PMG side fully complete + ai-skills leaf bootstrapped.** Session 2026-05-11 (continuation): assessment-pass redo of pmg-integrations trio (commit 7510942 on develop folded `.claude/CLAUDE.md` sidecar into root + added ARCH §6 Drift section); pmg-docs assessment pass (commit ab63466 on main added ARCH §6 + CLAUDE.md "Honesty About Drift" pointer); pmg-chatwoot assessment redo (commit 590d76ddf on `feature/cross-repo-init-trio` replaced CLAUDE.md→AGENTS.md symlink with real PMG-canonical CLAUDE.md folded from sidecar, AGENTS.md kept for non-Claude agents, fixed wrong "upstream untouched" framing in ARCH §2/§4.3 and CROSS-REPO §4); pmg-docs catch-up commit 6c74f8f (4 PRDs archived + infra doc updates pushed to main); ai-skills trio bootstrapped on main (this commit). PRs open: pmg-integrations #68, pmg-chatwoot #115. Earlier in session, Phase 5 prior history: §8.1 (pmg-integrations on develop, commit 0befa30) + §8.2 PMG side (pmg-docs on main, commit 19bd2f0) + §8.2 WellMed side (kalpa-docs on main, commits 0950290 + 330bd22 + **4c03f09** fold/archive) + §8.3 (wellmed-infrastructure on develop, commit cd2acac + ca16267 re-trunk-fold + 1b673ac port-fix) + **§8.4 ALL WellMed leaves complete** (wellmed-backbone develop 5758f21+acf2a2d; wellmed-consultation develop d115818; wellmed-cashier develop 16abb9c; wellmed-pharmacy develop 939dd88; wellmed-gateway-go develop 0883e59; wellmed-hq develop 84436af; wellmed-supply-chain develop 7fa0549; wellmed-fe develop aa9ec41; wellmed-hq-fe main b6b8450; wellmed-catalog main e0ab883 STUB; kalpa-company-profile main 319c4ca). §8.3 prep was done in prior session: Kalpa-Health gap analysis + cleanup; wellmed-gateway-go ingarso remote removed; 6 behind-repos fast-forwarded to org defaults; 5 gap repos cloned; wellmed-consultation-shared confirmed retired (memory saved); all 13 local wellmed repos ahead=0 behind=0. The branch-aware surveying logic baked into /cross-repo-init pre-§8.3 paid for itself immediately: wellmed-infrastructure showed +0 on main / +10 on develop with ~19k lines of code (SDK + adapter library + installpb + migration/seeder) absent from main; the §2.2 cascade correctly routed survey to origin/develop. All three feature branches (`feat/sdk-restructure-opsi3`, `feature/shared-go-sdk`, `feat/pharmacy-ssm-parameters`) confirmed fully merged into develop (right-side count = 0) — safe to delete remotely but left for user. Repo now has CROSS-REPO.md (100 lines, trunk archetype with 10 consumers + kalpa-docs docs leaf + 4 external dependents), ARCHITECTURE.md (268 lines with the load-bearing §6 Drift section documenting main-vs-develop), CLAUDE.md (206 lines at repo root, supersedes stale `.claude/CLAUDE.md` from 2026-03). Committed locally on develop only — not pushed per workflow.
**Next action:** Phase 5 dogfood pass is functionally complete (all 13 PMG/WellMed/ai-skills repos bootstrapped). Open items: (1) wait for PRs #68 (pmg-integrations) and #115 (pmg-chatwoot) to merge — then local develop in each will sync via `git pull`; (2) run `/closeout` on the closeout-skills scope itself to consume `plans/closeout-skills/closeout-skills-PROGRESS.md`, archive the scope to `plans/archive/`, update `plans/PLANS-INDEX.md`, and write memory entries for v1.1 calibration findings (see ai-skills `ARCHITECTURE.md §6.3` for the list); (3) optionally exercise the verification recipes in `plan/tests/verification-recipes.md` and `closeout-extended/tests/upward-traversal-recipe.md` against pmg-integrations now that the trio is real.
**Open blockers:** None. All three orgs fully bootstrapped; PRs #68 and #115 awaiting review and merge.

### Session 2026-05-11 (continuation): PMG-side assessment passes + ai-skills leaf

**Per user direction "do all three in order, then ai-skills, assessment pass":**

- **pmg-integrations** (commit 7510942 on develop, later moved to `feature/cross-repo-init-trio` PR #68):
  - Assessment found a pre-existing `.claude/CLAUDE.md` (250+ lines, May 8) that the prior §8.1 work missed because the trio was created without inspecting that path. The sidecar was richer than the new root CLAUDE.md (bus IPC pattern, ports 3012/3013, full SSM parameter table, connector manifest spec, Day 0 service details, known issues #1-#5).
  - Folded sidecar content into root CLAUDE.md (112 → 331 lines on the 12-section numbered template). Deleted `.claude/CLAUDE.md`. ARCHITECTURE.md grew 178 → 282 lines: thickened §1 Components, new §2.1 parent/child fork ASCII, new §6 Drift section. CROSS-REPO.md unchanged (already healthy). Reconciled branch workflow: develop = dev trunk; main = deploy branch (GitHub Actions self-hosted runner triggers on push to main).
  - PR opened against develop (PR #68). Local develop reset to origin/develop; feature branch carries both `0befa30` (original bootstrap) + `7510942` (fold-up).

- **pmg-docs** (commit ab63466 on main + earlier `6c74f8f` catch-up, both pushed to origin):
  - Lighter touch — no sidecar to fold. Added ARCHITECTURE.md §6 "Current Information State vs Target Layout" (43 lines) and CLAUDE.md "Honesty About Drift" pointer (9 lines). Verified all top-level dirs in §1 still exist; ADR/postmortem naming conventions still honored.
  - Catch-up commit 6c74f8f bundled 11 uncommitted files (4 PRDs moved to `development/prds/archive/`, 5 infra updates, 3 misc doc tweaks) — not gem-related; pushed direct to main per docs-leaf workflow.

- **pmg-chatwoot** (commits af623b633 [from earlier session] + 590d76ddf [this session] on `feature/cross-repo-init-trio` → PR #115):
  - The earlier `af623b633` commit had created `.claude/CLAUDE.md` (207 lines) + ARCHITECTURE.md (253 lines) + CROSS-REPO.md (104 lines) on the false premise that the root CLAUDE.md (a symlink to AGENTS.md) was untouched-upstream. Reality: AGENTS.md was already heavily PMG-customized (Sidekiq topology with 2026-05-05 staging-Redis incident, four PMG flag positions 35/36/41/47 with bigint-overflow reclaim rule, UI Design System reference, Codex worktree workflow).
  - Per user clarification: AGENTS.md is for non-Claude agents (Codex, Cursor) and stays untouched. The fix: replace the CLAUDE.md → AGENTS.md symlink with a real PMG-canonical Claude Code agent doc, fold the sidecar content into it (305 lines, 14 numbered sections — three-remote rules, flag table with reclaim rule + verification command, `// PMG:` marker conventions, SSM rules including per-env-key fallback, Sidekiq + Redis topology with the 2026-05-05 incident, deployment runbook with seven sub-rules, chatwoot-services/router/, WhatsApp NAMED format, pattern grep targets, workflow conventions, plans + memory pointers). Deleted `.claude/CLAUDE.md`; `.claude/settings.local.json` preserved.
  - Also fixed wrong "upstream untouched, do not modify" framing in ARCHITECTURE.md §2 directory tree + §4.3 Key Decisions, and CROSS-REPO.md §4 note.
  - Husky pre-push blocks direct push to develop → feature branch + PR was required. PR #115 opened against develop. Local develop reset to origin/develop.

- **ai-skills** (this commit, on main, solo-dev direct):
  - Bootstrapped trio from scratch — first time the framework is being dogfooded on its own source repo.
  - CROSS-REPO.md (74 lines): standalone-leaf archetype; no upstream Pattern Sources; no contract Consumers; operational consumers are `~/.claude/skills/*` symlinks (not git-traversable); cross-skill reference notes for `gstack` sibling repo and the `kalpa/` subdirectory holding WellMed-project-specific skills. max-depth-override: 0.
  - ARCHITECTURE.md (181 lines): §1 categorizes skills (planning + execution / document-style / project-specific / shared resources / repo-local planning state); §2 Data Flow ASCII showing skill invocation lifecycle via symlinks; §3 Key Decisions (SKILL.md frontmatter contract, numbered inline questions ban on AskUserQuestion, ASCII-only diagrams, templates/ shared vs `<skill>/templates/` private, solo-dev direct-to-main, symlinks not copies, cross-skill reuse not duplication); §5 cross-repo position (standalone, PMG/WellMed boundary irrelevant); §6 Drift table listing every skill with status (stable / new / recently-updated) plus v1.1 calibration items deferred.
  - CLAUDE.md (193 lines): 9-section template variant tuned for the skills-repo shape (no env vars / no build-test-lint sections — explicitly marked N/A and explained why). §3 architecture decisions emphasize SKILL.md frontmatter contract + AskUserQuestion ban + cross-skill reuse rule. §7 "What NOT to do" calls out the forks-vs-reuse trap, the SKILL.md frontmatter contract, and the gstack-don't-touch rule. §9 plans + memory pointers cover the closeout-skills scope and the v1.1 calibration findings.

**Key calibration finding from this session:** The assess-first-don't-sidecar pattern that surfaced during the WellMed §8.4 dogfood pass was exactly the right lesson — both pmg-integrations and pmg-chatwoot had pre-existing rich content that the earlier "always create sidecar" path silently created duplication around. The pmg-chatwoot case is particularly instructive: the prior session's bootstrap explicitly asserted "do not modify CLAUDE.md / AGENTS.md — they're upstream-untouched," when in fact both were heavily PMG-customized. v1.1 of `/cross-repo-init` Step 2 needs to: (a) check for `.claude/CLAUDE.md` at sidecar path; (b) for root CLAUDE.md / AGENTS.md, scan for PMG/WellMed/project-specific content before treating as upstream-pristine; (c) propose fold direction explicitly with numbered options rather than defaulting to sidecar creation. The current `cross-repo-init/SKILL.md` v1.1 already has part of this baked in (post-WellMed-dogfood); the chatwoot blunder shows it's not yet fully closed.
